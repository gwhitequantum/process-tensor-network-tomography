...
# This file includes modifications based on the work of [Johnnie Gray] (https://github.com/jcmgray/quimb)
...
from quimb.tensor.array_ops import iscomplex
...
from quimb.utils import ensure_dict, valmap
...
class Vectorizer:
    ...
    def __init__(self, arrays):
        ...
        self.d = sum(
            (1 + int(cmplx)) * size for size, cmplx in zip(self.sizes, self.iscomplexes)
        )
        self.pack(arrays)
    ...
    def pack(self, arrays, name="vector"):
        ...
        if not hasattr(self, name):
            setattr(self, name, np.empty(int(self.d), "float64"))
        x = getattr(self, name)
    ...
    def unpack(self, vector=None):
        ...
        i = 0
        arrays = []
        for shape, size, cmplx, dtype in zip(
            self.shapes, self.sizes, self.iscomplexes, self.dtypes
        ):
            if not cmplx:
...
class JaxHandler:
    ...
    def setup_fn(self, fn):
        ...
        if self.jit_fn:
            ...
            self._value_and_grad = jax.jit(jax.value_and_grad(fn), backend=self.device)
        else:
            self._backend_fn = fn
    ...
    def value(self, arrays, cust_kws):
        jax_arrays = tuple(map(self.to_constant, arrays))
        return to_numpy(self._backend_fn(jax_arrays, cust_kws))

    def value_and_grad(self, arrays, cust_kws):
        loss, grads = self._value_and_grad(arrays, cust_kws)
...
class TorchHandler:
    ...
    def _setup_backend_fn(self, arrays):
        ...
        if self.jit_fn:
            ...
            self._backend_fn = torch.jit.trace(self._fn, example_inputs=example_inputs)
        else:
...
class MultiLossHandler:
    ...
    def _value_par_seq(self, arrays):
        futures = [self.executor.submit(h.value, arrays) for h in self.handlers]
    ...
    def _value_and_grad_par(self, arrays):
        futures = [
            self.executor.submit(h.value_and_grad, arrays) for h in self.handlers
        ]
...
class ADAM:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        return self.OptimizeResult(
            x=x, fun=fun(x, *args), jac=g, nit=self._i, nfev=self._i, success=True
        )
...
class MakeArrayFn:
    ...
    def __call__(self, arrays, cust_kws={}):
        ...
        with contract_backend(self.autodiff_backend):
            return self.loss_fn(self.norm_fn(tn_compute), **cust_kws)
...
def generate_random_TP_keys(nQ, nSamples):
    # at the moment just keep this one X gate and one POVM (can generalise later on)
    nSteps = 2  # this will be what changes when the number of ops changes
    nqubits = 2 * nSteps
    rand_keys = []
    for i in range(nSamples):
        tmp = []
...
class TNOptimizer:
    ...
    def __init__(
        ...
        **backend_opts,
        ...
        self._nData = int(len(training_data))
        if batch_size:
            self.batch_size = int(batch_size)
            self._nBatches = int(self._nData / self.batch_size)
        else:
        ...
        self._Lx = Lx
        self._Ly = Ly
        self.causality_key_size = causality_key_size
        self.causality_fn = causality_fn
        self.TP_key_size = TP_key_size
        self.TP_fn = TP_fn

        # extract the individual tensors from the sequence TN

        if batch_size:
            self.training_sequences = training_sequences
        self.validation_data = validation_data
        self.validation_sequences = np.array(validation_sequences)
        ...
        self.handler_val = deepcopy(self.handler)
    ...
    def _maybe_update_pbar(self):
        if self._pbar is not None:
            ...
            self.val_best = min(self.val_best, self.val_loss)

            msg = f"{self.loss:+.7f} [best loss: {self.loss_best:+.7f}] [best val: {self.val_best:+.7f}; ({np.argmin(self.val_losses)})]"
    ...
    def vectorized_value_and_grad(self, x, cust_kws={}):
        """The value and gradient of the loss function at vector ``x``."""
        self.vectorizer.vector[:] = x
        arrays = self.vectorizer.unpack()
        result, grads = self.handler.value_and_grad(arrays, cust_kws)
        self._n += 1
    ...
    def vectorized_value_and_grad_batch(self, x, cust_kws={}):
        """The value and gradient of the loss function at vector ``x``."""

        keys = np.random.choice(range(self._nData), self.batch_size, replace=False)
        tmp_data = self.training_data[keys]
        tmp_seqs = self.training_sequences[keys]

        causality_keys = generate_random_causality_keys(
            self._Ly - 1, self._Lx, self.causality_key_size
        )
        causality_arrays = self.causality_fn(causality_keys)

        cust_kws["cArrays"] = causality_arrays
        cust_kws["sequence_list"] = tmp_seqs
        cust_kws["data"] = tmp_data

        if self.TP_fn:
            TP_keys = generate_random_TP_keys(self._Lx, self.TP_key_size)
            TP_arrays = self.TP_fn(TP_keys)
            cust_kws["tArrays"] = TP_arrays
        ...
        if self.validation_data:
            if self.TP_fn:
                self.val_loss = self.handler_val.value(
                    arrays,
                    {
                        "sequence_list": self.validation_sequences,
                        "data": self.validation_data,
                        "kappa": 1,
                        "cArrays": causality_arrays,
                        "tArrays": TP_arrays,
                    },
                )
            else:
            ...
            if self.val_loss < self.val_best:
                self.best_val_mpo = self.get_val_tn_opt(arrays)

        self._n += 1

        self._nB += 1
        self._nB = self._nB % self._nBatches
        self.loss = result.item()
        self.losses.append(self.loss)
        vec_grad = self.vectorizer.pack(grads, "grad")
        self._maybe_update_pbar()
    ...
    def __repr__(self):
        return f"<TNOptimizer(d={self.d}, " f"backend={self._autodiff_backend})>"
    ...
    def get_val_tn_opt(self, arrays):
        """Extract the optimized tensor network, this is a three part process:
    ...
    def optimize(self, n, tol=None, jac=True, hessp=False, cust_kws={}, **options):
        ...
        if jac and self.batch_size:
            fun = self.vectorized_value_and_grad_batch

        elif jac and not self.batch_size:
    ...
    def optimize_basinhopping(
        self, n, nhop, temperature=1.0, jac=True, hessp=False, cust_kws={}, **options
    ):
...
class TNOptimizer_circ:
    ...
    def optimize(self, n, tol=None, jac=True, hessp=False, **options):
        """Run the optimizer for ``n`` function evaluations, using
        :func:`scipy.optimize.minimize` as the driver for the vectorized
        computation. Supplying the gradient and hessian vector product is
        controlled by the ``jac`` and ``hessp`` options respectively.
